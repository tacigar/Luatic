...
namespace luatic {
    ...
    private:
        ...
        public:
            ...
            auto defFunction(const std::string& functionName,
                             R(*functionBody)(Args...))
                -> Namespace_<Global_<M>, N + 1> {
                DefFunction<Namespace_<Global_<M>, N>, R, Args...>::apply
                    (_L, functionName, functionBody);
                lua_setfield(_L, -2, functionName.data());
                return Namespace_<Global_<M>, N + 1>(_L, _name);
            }
        ...
        public:
            ...
            auto end() -> Namespace_<Outer_, M + 1> {
                lua_setglobal(_L, _name.data());
                return Namespace_<Outer_, M + 1>(_L, _name);
            }
            
            template <class R, class... Args>
            auto defFunction(const std::string& functionName,
                             R(*functionBody)(Args...))
                -> Namespace_<Namespace_<Outer_, M>, N + 1> {
                DefFunction<Namespace_<Namespace_<Outer_, M>, N>, R, Args...>::apply
                    (_L, functionName, functionBody);
                lua_settable(_L, -3);
                return Namespace_<Namespace_<Outer_, M>, N>(_L, _name);
            }
    ...
    class Namespace {
    public:
        Namespace(lua_State* L, const std::string& name)
            : _L(L), _name(name) {
        }
        ~Namespace() = default;
        
        auto begin() -> detail::Namespace_<Outer, 0> {
            lua_newtable(_L);
            return detail::Namespace_<Outer, 0>(_L, _name);
        }
